...
## 2026-06-11 — Remaining credits links + MIS Bulk Alt Report
- Fixed the 7 remaining pages whose link text to /managing-credit-usage still read "How credits work" → now "Managing credit usage": what-is-a-job, the-universal-llm-key, deployment-plan-levels, payment-methods-regional-billing, cancellation-refunds, talk-it-through, watch-your-app-come-alive. Verified 0 "How credits work" links to that slug remain. (Contextual anchors like "credit balance"/"credit allocation" intentionally left unchanged.)
- MIS Bulk Alt Report: mis_dashboard now returns images_missing_alt [{slug,title,count,status}] (uses count_images_missing_alt). MIS tab renders "Images missing alt text (N pages — blocks publishing)" card: rose count badges, status pill, click-through to editor, CSV export (exportAltCsv). Verified E2E. NOTE: report surfaced a genuine pre-existing case — figma-design-to-code has 1 image missing alt (left as-is; real content to fix later).

## 2026-06-11 — Delegate produces per-page assignments (bug fix)
- Bug: the delegate panel ("Delegate all"/delegate-pages) merged every delegated slug into ONE scope_type="pages" bundle ("Delegated pages · N page(s)"), so the target saw a single review covering all pages instead of N individual reviews. Activity: vinish@emergent.sh delegated 27 pages to akhil.t@emergent.sh (2026-09-11) via delegate-pages.
- Fix: delegate_pages (review_routes.py) now creates ONE scope_type="page" assignment per slug for the target (title as scope_label, delegated_from set, one-assignee-per-page enforced) instead of a bundle.
- Migration: split akhil.t's existing 27-slug "Delegated pages" bundle into 27 individual page assignments (and any other scope_type="pages" bundle). Verified: akhil.t = 27 individual 'page' assignments, 0 bundles; UI shows "akhil.t · 27 review(s)". delegate-bulk (per-reviewer "Delegate all") already preserved structure and is unchanged.
...
## 2026-06-11 — Single verdict per page + verdict history
- Model change (per user): ONE verdict per page (reviewer OR owner), latest wins — not one-per-user. set_verdict (review_routes.py) now upserts on {project_id, doc_slug} and records reviewer_email/name = whoever last set it. Owner can change it; the change is captured in activity history.
- New endpoint GET /projects/{pid}/verdict-history?doc_slug= returns the activity_log "verdict" timeline (who set what, when).
- Migration: collapsed review_verdicts to one record per (project, slug), keeping latest by updated_at (17 pages kept, 5 duplicate reviewer/owner records removed).
- ReviewPage.jsx: panel relabeled "Verdict" (was "Your verdict"); shows current single verdict + "Set by <name> · <time>" + "View verdict history" toggle listing the timeline. Progress/reviewedSet now = pages whose verdict is "Looks correct" (any setter). Removed the earlier dual-verdict "Reviewer's verdict" block. Verified: owner override switches the single verdict and appends a history entry; restored test data afterwards.
