    ...
    <div className="h-screen overflow-hidden bg-background flex flex-col" data-testid="docs-view-page">
      ...
      <header className="h-14 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between px-4 bg-background sticky top-0 z-50">
        <div className="flex items-center gap-3">
          <button 
            className="lg:hidden p-2 -ml-2 text-zinc-600 dark:text-zinc-400 hover:text-zinc-950 dark:hover:text-zinc-950 dark:text-white"
          ...
          <span className="text-zinc-950 dark:text-white font-medium">{project?.name}</span>
        </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className={`
          ...
          lg:relative lg:block lg:w-72 bg-background border-r border-zinc-200 dark:border-zinc-800 flex-shrink-0
        `}>
                    ...
                    return (
                      ...
                      >
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end" className="bg-white dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800 w-32">
                            <DropdownMenuItem 
                              className="text-zinc-600 dark:text-zinc-400 focus:text-zinc-950 dark:focus:text-zinc-950 dark:text-white focus:bg-zinc-100 dark:bg-zinc-800 cursor-pointer text-sm"
                              onClick={(e) => {
                                e.stopPropagation();
          ...
          {selectedDoc ? (
            <div className="max-w-4xl mx-auto px-6 lg:px-12 py-10 page-transition">
              ...
              <div className="flex items-center gap-2 text-sm text-zinc-500 mb-6">
                <button onClick={() => navigate("/admin/dashboard")} className="hover:text-zinc-950 dark:hover:text-zinc-950 dark:text-white transition-colors">
                  <Home className="w-4 h-4" />
                </button>
                <ChevronRight className="w-3 h-3" />
                <span>{project?.name}</span>
                <ChevronRight className="w-3 h-3" />
                <span className="text-zinc-700 dark:text-zinc-300">{selectedDoc.title}</span>
              </div>
              ...
              <div className="flex items-start justify-between mb-8">
                <div className="flex items-center gap-3">
                  ...
                  <h1 className="text-3xl font-bold text-zinc-950 dark:text-white tracking-tight">{selectedDoc.title}</h1>
                </div>
                <button
                  onClick={() => navigate(`/admin/editor/${projectId}/${selectedDoc.id}`)}
                  className="p-2 text-zinc-500 hover:text-zinc-950 dark:hover:text-zinc-950 dark:text-white transition-colors"
            ...
            <div className="flex items-center justify-center h-full">
              <div className="text-center">
                ...
                <h3 className="text-lg font-medium text-zinc-950 dark:text-white mb-2">No documents yet</h3>
        ...
        {selectedDoc && toc.length > 0 && (
          <aside className="hidden xl:block w-56 flex-shrink-0 border-l border-zinc-200 dark:border-zinc-800 p-6">
            ...
            <nav className="space-y-2">
              {toc.map((item, i) => (
                <a
                  key={i}
                  href={`#${item.id}`}
                  className={`block text-sm text-zinc-600 dark:text-zinc-400 hover:text-zinc-950 dark:hover:text-zinc-950 dark:text-white transition-colors ${
                    item.level === 3 ? 'pl-3' : ''
                  }`}
                >
                  {item.text}
      ...
      <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <AlertDialogContent className="bg-white dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-zinc-950 dark:text-white">Delete document?</AlertDialogTitle>
